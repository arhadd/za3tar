...
/** how Za3tar read the ask box before enter was pressed (jev.rs) */
export type AskIntent = "catch_up" | "write" | "do" | "nudge" | "question";
export type AskRead = {
  intent: AskIntent;
  intent_confidence: number;
  workspace: string;
  workspace_confidence: number;
  thread: string;
  thread_confidence: number;
  outside: number;
  /** route id when one hand should take this, else "" */
  hand: string;
...
export type Settings = {
  ...
  za3tar_base: string;
  onboarded: string;
  /** secrets never come back from Rust: this says which are set (last four) */
  keys?: Record<string, { has_key: boolean; last4: string }>;
  /** secret fields the user asked to remove on the next save */
  clear?: string[];
};
