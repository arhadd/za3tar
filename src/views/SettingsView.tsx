  ...
  return (
    <div className="flex max-w-xl flex-col gap-4">
      ...
      <Card className="flex flex-col gap-3">
        ...
        <SecretField
          label="ElevenLabs API key (transcription)"
        ...
        />
        <SecretField
          label="Anthropic API key (notes, decisions, drafts)"
          field="anthropic_api_key"
          form={form}
          onChange={onChange}
        />
        <SecretField
          label="TypeSafe API key (optional — reads the ask box as you type)"
...
/** A key field that never shows the stored key: it says whether one is set
 *  (and its last four), an empty box keeps it, "remove" clears it on save. */
function SecretField({
  ...
  label: string;
  field: "elevenlabs_api_key" | "openai_api_key" | "anthropic_api_key" | "typesafe_api_key";
  form: Settings;
  onChange: (s: Settings) => void;
}) {
  const hint = form.keys?.[field];
  const clearing = form.clear?.includes(field) ?? false;
  const saved = hint?.has_key && !clearing;
  return (
    <label className="flex flex-col gap-1 text-[12px] text-olive">
      <span className="flex items-baseline gap-2">
        ...
        {saved && (
          <button
            ...
            onClick={() =>
              onChange({ ...form, [field]: "", clear: [...(form.clear ?? []), field] })
            }
          >
            remove
          </button>
      ...
      <Field
        ...
        placeholder={
          saved
            ? `saved${hint?.last4 ? ` ····${hint.last4}` : ""} — leave empty to keep`
            : clearing
              ? "removed on save"
              : ""
        }
        onChange={(e) =>
          onChange({
            ...
            clear: (form.clear ?? []).filter((c) => c !== field),
          })
        }
      />
    </label>
