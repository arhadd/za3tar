...
function App() {
  ...
  /** a hand-off the model proposed, waiting for the user to say send */
  const [routeConfirm, setRouteConfirm] = useState<{
    label: string;
    message: string;
  ...
  async function saveSettings() {
    ...
    try {
      // empty secret = keep the stored one; `clear` removes one explicitly
      const { keys: _keys, clear, ...settings } = settingsForm;
      await invoke("save_settings", { settings, clear: clear ?? [] });
    ...
    for (const op of ops) {
      try {
        if (op.op === "park" || op.op === "unpark" || op.op === "done") {
          ...
          await refreshAll();
          out.push(`${w.name} — noted what it is`);
  ...
  // ── routes ────────────────────────────────────────────────────────────
  async function ensureRoute(routeId: string, quiet = false): Promise<Route> {
    ...
    if (r.kind === "acp" && (!cur || cur.status === "closed")) {
      ...
      if (!quiet) setRouteOpen(routeId);
      await startRoute(r, r.cwd || undefined);
    }
    return r;
  }

  /** the hand Za3tar reaches for by default: the first enabled agent route */
  const defaultRoute = routes.find((r) => r.enabled && r.kind === "acp") ?? routes.find((r) => r.enabled);

  /** a typed ask that Jev read as "for <agent>": skip the Claude turn and
   *  hand it over as-is; the answer lands in chat like any other */
  async function handDirect(routeId: string, text: string) {
    const label = routes.find((r) => r.id === routeId)?.label ?? routeId;
    if (!(await confirmRoute(routeId, text))) {
      // not handed off: treat it as an ordinary ask instead
      setChatMode("chat");
      setChatLines([]);
  ...
  // Za3tar's where-it-stands lines go stale the moment something moves in a
  // chat the app cannot see. The agent can see it. So on launch (and on
  // demand) the app asks the default route for a fresh line per active
  // thread and keeps only what changed. Quiet: no panel opens unless the
  // agent asks for a permission.
  const SYNC_KEY = "za3tar.threads.synced";
  ...
  async function syncThreadLines(force = false) {
    const r = defaultRoute;
    if (!r || syncing) return;
    const active = threads.filter((t) => t.status === "active");
    if (!active.length) return;
    if (!force && Date.now() - syncedAt < 15 * 60 * 1000) return;
    setSyncing(true);
    try {
      await ensureRoute(r.id, true);
      const wsOf = (id: string) => workspaces.find((w) => w.id === id)?.name ?? id;
      const prompt = [
        "Za3tar (my workspace app on the Mac) keeps ONE where-it-stands line per thread. Refresh them from what you know right now: your THREADS.md, your notes, and the latest in the relevant chats.",
        "Reply with ONLY a JSON object mapping thread id to a new line (one or two sentences, present tense: what stands and what is open, names kept as written). Leave out any thread you have nothing newer or more precise on than the current line. No prose outside the JSON.",
        "",
      ...
      if (a < 0 || b <= a) throw new Error(`${r.label} sent no JSON`);
      const lines = JSON.parse(raw.slice(a, b + 1)) as Record<string, unknown>;
      let n = 0;
      for (const t of active) {
        const line = lines[t.id];
        if (typeof line !== "string") continue;
        const summary = line.trim();
        if (!summary || summary === t.summary.trim()) continue;
        await invoke("update_thread", {
          id: t.id,
          title: t.title,
          summary,
  ...
  // once the app has its threads and routes: warm the default hand and
  // refresh the lines if they are older than 15 minutes
  useEffect(() => {
    if (syncTried.current || !loaded || !defaultRoute || !threads.length) return;
    syncTried.current = true;
    void syncThreadLines().then(() => {
      if (defaultRoute.kind === "acp") void ensureRoute(defaultRoute.id, true).catch(() => {});
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loaded, defaultRoute?.id, threads.length]);

  // a quiet session that needs a permission answered: show it
  useEffect(() => {
    if (routeOpen) return;
    const waiting = Object.values(routeSessions).find((s) => s.permission);
    if (waiting) setRouteOpen(waiting.route);
  }, [routeSessions, routeOpen]);

  /** Nothing the model proposes leaves the app on its own: show the target
   *  route and the exact message, and send only on an explicit click. */
  function confirmRoute(routeId: string, message: string): Promise<boolean> {
    const label = routes.find((r) => r.id === routeId)?.label ?? routeId;
    return new Promise((resolve) => {
      setRouteConfirm((cur) => {
        cur?.resolve(false); // a newer proposal replaces an unanswered one
        return { label, message, resolve };
      });
    });
  }
  function answerRouteConfirm(ok: boolean) {
    setRouteConfirm((cur) => {
      cur?.resolve(ok);
      return null;
    });
  ...
  /** a model-proposed hand-off: confirm, then ask */
  async function askRouteConfirmed(routeId: string, text: string): Promise<string | null> {
    if (!(await confirmRoute(routeId, text))) return null;
  ...
  /** fast pre-read of a typed ask (Jev): kind, workspace, thread. Null when
   *  no key is set or the read fails — the ask still works without it. */
  async function readAsk(text: string): Promise<AskRead | null> {
    if (!caps?.fast) return null;
    try {
      return await invoke<AskRead>("read_ask", {
        text,
        workspaces: workspaces.map((w) => ({
          ...
          description: w.description,
          people: people
            .filter((p) => p.workspaces.includes(w.id))
            .map((p) => [p.name, p.org].filter(Boolean).join(" of ")),
        })),
        threads: threads
          .filter((t) => t.status === "active")
          .map((t) => ({ id: t.id, workspace: t.workspace, title: t.title, summary: t.summary })),
        routes: routes
          .filter((r) => r.enabled)
          .map((r) => ({ id: r.id, label: r.label, description: r.description })),
        currentWorkspace: view === "home" ? null : wsId,
      });
    } catch {
      return null;
    }
  ...
  /** ask Za3tar straight from Home: no opener, just the question. With a
   *  confident pre-read, a "write" ask opens the Composer directly on the
   *  right thread instead of going through a chat turn first. */
  function askZa3tar(text: string, read?: AskRead) {
    if (read?.hand && read.hand_confidence >= 0.75 && routes.some((r) => r.id === read.hand && r.enabled)) {
      void handDirect(read.hand, text);
      return;
    }
    if (read && read.intent === "write" && read.intent_confidence >= 0.7) {
      const t = threads.find((x) => x.id === read.thread && read.thread_confidence >= 0.6);
      if (read.workspace && read.workspace_confidence >= 0.6) setWsId(read.workspace);
      void openComposer(t ? t.title : "draft", text);
      if (t) setComposer((cur) => (cur ? { ...cur, thread: t.id } : cur));
      return;
    }
    setChatMode("chat");
    setChatLines([]);
    void runChat(text, [], "chat", read);
  }
      ...
      const modeLine =
        mode === "onboarding"
          ...
          : mode === "sorting"
            ...
            : "";
      const readLine = read
        ? `PRE-READ of the ask (fast model, may be wrong): kind=${read.intent}${
            read.workspace && read.workspace_confidence >= 0.6
              ? ` · workspace=[${read.workspace}] ${workspaces.find((w) => w.id === read.workspace)?.name ?? ""}`
              : ""
          }${
            read.thread && read.thread_confidence >= 0.6
              ? ` · thread=[${read.thread}] ${threads.find((t) => t.id === read.thread)?.title ?? ""}`
              : ""
          }${read.outside >= 0.75 ? " · likely needs hands outside the app" : ""}\n`
        : "";
      const turn = await invoke<{ say: string; ops: LiveOp[] }>("live_turn", {
        snapshot: modeLine + readLine + snap.current(),
        transcript,
        typed: true,
      });
  ...
  async function startTalk() {
    ...
    const s = new LiveSession({
      ...
      runtime: (route, m) => askRouteConfirmed(route, m),
  ...
  const confirmOverlay = routeConfirm && (
    <div className="fixed inset-x-0 bottom-6 z-50 mx-auto w-full max-w-md px-4">
      <Card className="flex flex-col gap-3 border-ink shadow-lg">
        <Eyebrow>Send to {routeConfirm.label}?</Eyebrow>
        <p className="whitespace-pre-wrap text-[13px] leading-relaxed text-ink">
          {routeConfirm.message}
        </p>
        <div className="flex gap-2">
          <Button tone="primary" size="sm" onClick={() => answerRouteConfirm(true)}>
            Send
          </Button>
          <Button tone="quiet" size="sm" onClick={() => answerRouteConfirm(false)}>
            Cancel
          </Button>
        </div>
      </Card>
