...
fn routes_path(app: &AppHandle) -> Result<PathBuf, String> {
    Ok(crate::paths::data_dir(app)?.join("routes.json"))
}

/// No route ships by default: a route runs a command on this machine, so the
/// user adds their own in Settings. An example of the shape (disabled):
///
/// ```text
/// id: "ops", kind: "acp", command: "ssh my-server hermes -p ops acp"
/// ```
fn defaults() -> Vec<Route> {
    Vec::new()
}

fn saved_routes(app: &AppHandle) -> Result<Vec<Route>, String> {
...
/// The command comes from the saved routes, never from the webview: the UI
/// names a route, Rust looks up what it runs.
#[tauri::command]
pub async fn acp_start(
    ...
    let saved = saved_routes(&app)?
        .into_iter()
        .find(|r| r.id == route)
        .ok_or_else(|| format!("no route named {route}"))?;
    if !saved.enabled {
        return Err(format!("route {route} is turned off"));
    }
    if saved.kind != "acp" {
        return Err(format!("route {route} does not speak ACP"));
    }
    let command = saved.command.clone();
    let cwd = cwd.or_else(|| (!saved.cwd.trim().is_empty()).then(|| saved.cwd.clone()));
